...
def get_overlaps(first, second):  # function for checking overlaps
    if not first.isdisjoint(second) or not second.isdisjoint(first):  # check for overlaps
        overlap_count.append(1)   # if true, enter 1
    else:                         # otherwise
        overlap_count.append(0)   # enter 0


fully_contain_count = []  # list to hold hits or misses
overlap_count = []        # list to hold hits or misses
...
with open("../input/04.txt", "r") as file:  # open file
    for line in file:                       # loop through lines
        ...
        get_overlaps(first, second)         # call function with both elves

print(sum(fully_contain_count))  # print answer 1
print(sum(overlap_count))        # print answer 2
